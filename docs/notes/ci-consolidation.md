...
| Hubu release build of `hubu` and `hubu-server` | One locked release build of `hubu`, `hubu-server`, `hubu-mcp-server`, `gongbu-server`, and `gongbu-mcp` on pushes to `main` | Superseded by all five production binaries without repeating the release-profile build on pull requests. The release workflow independently verifies the same five binaries before publication. `hubu-bench` and `gongbu-sandbox` remain development tools, not production binaries. |
