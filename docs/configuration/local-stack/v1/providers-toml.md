...
Live mode requires the two spend fields plus either a provider contract or at
least one complete raw target and matching raw pricing. Multiple approved
provider, model, and resolution targets can coexist; agents discover and select
among them at runtime without rewriting the stack profile. A mixed contract-bound/raw
...
| Attribute | Value |
| --- | --- |
| Required | Sandbox and generic/composite live mode; optional for exactly one provider contract with no raw targets or pricing |
| Supplied by | Generated by Hubu for sandbox; chosen by the operator for live mode |
...
in that contract-only shape, it must equal the frozen version. A composite
contract-bound-plus-raw catalog requires a new operator-owned immutable label.
...
unless `[[contract_bindings]]` supplies the target. All raw target fields are
