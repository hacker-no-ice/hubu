...
pub(super) fn tool_definitions() -> Vec<Value> {
    ...
    vec![
        ...
        json!({"name":"gongbu_create_execution","description":"Continue one authorized normalized operation into one Gongbu execution using its opaque continuation identifier and execution intent. Prefer a target_id returned by gongbu_list_execution_targets; the legacy raw target tuple remains accepted for compatibility.","inputSchema":{"type":"object","additionalProperties":false,"required":["schema_version","spend_auth_token_id","input","input_schema_version"],"oneOf":[{"required":["target_id"]},{"required":["workload_type","provider","adapter","model"]}],"properties":create_properties}}),
