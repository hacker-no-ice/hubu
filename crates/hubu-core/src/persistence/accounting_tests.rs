...
#[test]
fn mismatched_budget_rolls_back_every_settlement_write() {
    for mismatch in ["agent", "currency"] {
        let mut repo = SqliteGovernanceRepository::in_memory().unwrap();
        let now = Utc::now();
        let (claim, token, hold) =
            persist_claimed_executor_spend(&mut repo, now + Duration::minutes(15));
        repo.conn
            ...
            .unwrap();
        let sql = if mismatch == "agent" {
            "UPDATE budgets SET scope_id='another-agent'"
        } else {
            "UPDATE budgets SET currency='EUR'"
        };
...
fn canonical_backfill_marks_incomplete_evidence_instead_of_inventing_postings() {
    for missing in [
        ...
        match missing {
            ...
            "budget_agent" | "budget_scope" | "budget_currency" => {
                repo.conn
                    .execute_batch("DROP TRIGGER budgets_logical_properties_immutable;")
                    .unwrap();
                let sql = if missing == "budget_agent" {
                    "UPDATE budgets SET scope_id='another-agent'"
                } else if missing == "budget_scope" {
            ...
            "budget_version" => {
                repo.conn
                    .execute(
                        "UPDATE budget_holds SET budget_version_id='00000000-0000-0000-0000-000000000001'",
                        [],
                    )
                    .unwrap();
            }
