...
const components = {
  ...
  api: {
    ...
    responsibilities: [
      ...
      "Delegates authorize, reserve cap, reserve budget, persist token/holds, payment, and settle/release to `hubu-core::app` so the spend path is testable without HTTP.",
  ...
  app: {
    ...
    responsibilities: [
      ...
      "Reserves the user cap and agent budget as a single application flow, releasing compensation holds when a later step fails.",
      "Persists spend auth tokens and frozen holds only after both financial boundaries accept the request.",
      "Submits wallet payments, persists payment attempts, marks successful tokens used, and settles, releases, or keeps both holds frozen according to the failed-payment retry policy.",
    ...
    nodes: [
      ...
      { id: "reserve", label: "Reserve limits", sub: "cap + budget", x: 430, y: 282, w: 224, h: 92, tone: "core", path: "crates/hubu-core/src/budget/manager.rs" },
      { id: "persist", label: "Persist governance", sub: "decision/token/holds", x: 798, y: 188, w: 238, h: 98, tone: "data", path: "crates/hubu-core/src/persistence.rs" },
      ...
      { id: "settle", label: "Settle/release", sub: "both holds", x: 798, y: 444, w: 238, h: 98, tone: "core", path: "crates/hubu-core/src/app/spend_approval.rs" },
    ],
    edges: [
  ...
  budget: {
    ...
    kind: "Component",
    copy:
      "Budgets are execution-scoped allocations for agents today and tasks later. Humans create the agent budget and user cap, then the app service reserves one budget hold and one cap hold before Hubu payment completion or executor completion settles, releases, or preserves them for retry.",
    responsibilities: [
      ...
      "Reserves holds from the app service only after spend fits both an active agent budget and the active user cap.",
      "Reserves, settles, releases, keeps retryable failures frozen, and expires budget/cap holds for wallet payments and external executors.",
    ...
    links: [sharedLinks.budget, sharedLinks.budgetModel, sharedLinks.appSpend, sharedLinks.spendExecutor, sharedLinks.persistence, ["Budget DTOs", "crates/hubu-core/src/budget/dto.rs"]],
    nodes: [
      ...
      { id: "agentSpend", label: "App service", sub: "authorize request", x: 76, y: 248, w: 206, h: 92, tone: "core", path: "crates/hubu-core/src/app/spend_approval.rs" },
