const sharedLinks = {
  ...
  gongbuPricing: ["Gongbu pricing contract", "crates/gongbu-api/src/provider/contract.rs"],
...
const components = {
  top: {
    ...
    copy:
      "Agents use one default MCP surface. Its governed-execution tool can authorize, execute, observe, and deliver a normal auto-approved result in one bounded call, while Hubu and Gongbu retain separate credentials, storage, provider work, artifacts, and failure domains.",
    responsibilities: [
      ...
      "The unified MCP surface is the only agent-facing surface; its composite reuses Hubu authorization and the durable Gongbu worker without creating a second state machine or collapsing independently configured backend clients.",
    ],
    ...
    nodes: [
      ...
      { id: "mcp", label: "Unified MCP", sub: "one-call governed path", x: 340, y: 382, w: 230, h: 96, tone: "surface", path: "crates/hubu-unified-mcp/src/lib.rs" },
      ...
      { id: "gongbu", label: "Gongbu HTTP API", sub: "installation caller; no principal", x: 706, y: 570, w: 226, h: 104, tone: "executor", path: "crates/gongbu-api/src/http/mod.rs" },
    ...
    edges: [
      ...
      ["mcp", "gongbu", "worker + Gongbu tools", { fromSide: "right", toSide: "left", waypoints: [{ x: 610, y: 430 }, { x: 610, y: 622 }], labelSegment: 1, labelDx: 42 }],
      ["api", "app", "govern"],
      ["app", "ledger", "persist"],
  ...
  budget: {
    ...
    kind: "Component",
    copy:
      "Agent budgets are hard execution-scoped allocations; user spending targets are separate advisory records. Spend reserves exactly one agent-budget hold, converts final exact vendor cost to cents with checked ceiling, and keeps expired or over-limit uncertainty frozen until a human reconciles vendor billing.",
    responsibilities: [
    ...
    nodes: [
      ...
      { id: "store", label: "Governance store", sub: "exact receipts + charges", x: 780, y: 282, w: 230, h: 96, tone: "data" },
  ...
  gongbu: {
    ...
    copy:
      "Gongbu starts without an execution principal. Its installation caller authenticates the service, while each new Hubu authorization supplies the account and agent snapshot. Gongbu remains outside Hubu's process, database, credential, provider-execution, artifact, and failure boundaries.",
    responsibilities: [
      ...
      "Rejects unselectable target tuples and unmatched image-size pricing selectors with bounded reason-and-field diagnostics, and writes the same static diagnostics to process logs without request values.",
      ...
      "For new work, resolves Hubu's authorization snapshot read-only, takes its account and agent as authoritative, exact-matches Gongbu-derived target scope and catalog price, and persists the immutable snapshot before scheduling.",
      ...
      "Accepts only schema-v2 pricing catalogs and freezes the complete exact rational snapshot plus normalized image-size selectors; the retired flat pricing shape is rejected.",
      "Persists an immutable execution and provider attempt before crossing billable boundaries, then stores exact integer provider amount, decimal scale, currency, and receipt evidence before finalization and recovery through Gongbu-owned Temporal workflow state.",
      ...
      "Sends final exact cost and the full frozen snapshot to Hubu, which recomputes budget cents with checked ceiling and settles only within the normal authorization; ambiguous or over-limit outcomes retain exact attempt evidence and stay in reconciliation instead of causing blind provider retries.",
    ...
    links: [sharedLinks.gongbuOverview, sharedLinks.gongbuServer, sharedLinks.gongbuServerConfig, sharedLinks.gongbuApplication, sharedLinks.gongbuWorkflow, sharedLinks.gongbuExecution, sharedLinks.gongbuArtifact, sharedLinks.gongbuProvider, sharedLinks.gongbuPricing, sharedLinks.gongbuHubu, sharedLinks.unifiedMcp, sharedLinks.gongbuConfig, sharedLinks.spendExecutor, sharedLinks.executionScope, sharedLinks.api],
    zones: [
    ...
    nodes: [
      ...
      { id: "gongbuApi", label: "Execution API", sub: "principal-neutral caller", x: 340, y: 112, w: 210, h: 92, tone: "executor", path: "crates/gongbu-api/src/http/mod.rs" },
      ...
      { id: "executionDb", label: "Gongbu SQLite", sub: "exact receipts + replay", x: 340, y: 352, w: 210, h: 96, tone: "data", path: "crates/gongbu-api/src/execution/mod.rs" },
    ...
    edges: [
      ...
      ["gongbuApi", "hubu", "new: Hubu attribution", { fromSide: "right", toSide: "top", waypoints: [{ x: 580, y: 158 }, { x: 580, y: 450 }, { x: 1109, y: 450 }], labelSegment: 2, labelDy: -12 }],
      ["gongbuApi", "executionDb", "local replay / persist"],
      ["gongbuApi", "workflow", "schedule"],
      ["workflow", "temporal", "durable state"],
      ["workflow", "executionDb", "exact cost + snapshot", { labelDx: -36, labelDy: 26 }],
      ["workflow", "hubu", "ceil-settle / reconcile", { fromSide: "right", toSide: "left", waypoints: [{ x: 936, y: 158 }, { x: 936, y: 606 }], labelSegment: 1, labelDx: 120 }],
  ...
  cli: {
    ...
    nodes: [
      ...
      { id: "profile", label: "Stack profile", sub: "topology; no principal", x: 380, y: 72, w: 220, h: 92, tone: "data", path: "crates/hubu-cli/src/stack.rs" },
  ...
  mcp: {
    ...
    viewBox: "0 0 1280 760",
    copy:
      "The agent harness launches one default stdio server. The router offers one bounded governed-execution call over two isolated HTTP backends, reusing its durable normalized-operation worker while leaving governance, provider execution, backend storage, credentials, artifacts, and failures with their owners.",
    responsibilities: [
      ...
      "Publishes the four accepted gongbu_* execution and artifact primitives, local hubu_operation_status, and router-owned hubu_submit_governed_execution with stable schemas, private continuation binding, public-handle correlation, and recursive redaction.",
      ...
      "Returns approval_required immediately without Gongbu or provider work; a nonterminal execution at budget expiry returns in_progress while terminal success stays succeeded and reports any artifact-delivery warning.",
      ...
      "Publishes a generic client approval profile so any harness can auto-approve reads, spend submission, and composite governed submission but prompt before resolving a needs_approval decision.",
      ...
      "Persists one stable local installation identity and atomically allocates an independent private backend operation key and immutable public operation_handle alongside a canonical tool-and-argument request hash; exact redelivery recovers while identity collisions fail before backend access.",
      "Marks dispatch before Hubu mutation, stores monotonic terminal results and the sanitized replay payload separately from decision and continuation columns, and retains private keys only inside trusted adapter state for continuation verification and recovery.",
      ...
      "Exposes only the public operation handle through hubu_operation_status; acknowledged work is never replacement-safe, a denied handle remains terminal while corrected work uses a new logical operation, and adapter reconciliation exhaustion terminates as failed while Gongbu retains its independent unresolved financial record.",
      ...
      "Returns the stable public handle with decision-aware guidance: ambiguous results must reuse the same harness call identity, while a definitive denial translates backend key-reuse guidance into a new harness call and logical operation for corrected work.",
    ...
    links: [sharedLinks.unifiedMcp, sharedLinks.unifiedGovernedExecution, sharedLinks.unifiedMcpStdio, sharedLinks.unifiedMcpNotifications, sharedLinks.unifiedHubuCatalog, sharedLinks.unifiedHubuRouting, sharedLinks.unifiedOperationRegistry, sharedLinks.unifiedOperationWorker, sharedLinks.unifiedGongbuCatalog, sharedLinks.unifiedGongbuFixture, sharedLinks.unifiedMcpContract, sharedLinks.api, sharedLinks.gongbuApplication],
    zones: [
    ...
    nodes: [
      ...
      { id: "tools", label: "Static router", sub: "35 tools; 1 composite", x: 330, y: 92, w: 200, h: 96, tone: "surface", path: "crates/hubu-unified-mcp/src/lib.rs" },
      ...
      { id: "operationRegistry", label: "Operation registry", sub: "lifecycle + replay request", x: 650, y: 334, w: 200, h: 96, tone: "data", path: "crates/hubu-unified-mcp/src/operation_registry.rs" },
    ...
    edges: [
      ["agent", "tools", "one call + public handle", { labelDy: -54 }],
      ["notifications", "agent", "list_changed", { fromSide: "left", toSide: "right", labelDy: -18 }],
      ["agent", "capability", "status", { labelDy: 48 }],
      ["tools", "hubuClient", "Hubu + auth"],
      ["tools", "operationRegistry", "normalize + bind"],
      ["operationRegistry", "operationWorker", "", { fromSide: "left", toSide: "right" }],
      ["operationWorker", "gongbuClient", "safe create + GET"],
      ["operationRegistry", "hubuClient", "private backend key"],
      ["tools", "gongbuClient", "reads / artifacts", { fromSide: "bottom", toSide: "top", waypoints: [{ x: 430, y: 350 }, { x: 750, y: 350 }], labelSegment: 1 }],
  ...
  agent: {
    ...
    kind: "Flow",
    copy:
      "Agents never hold private backend operation keys. For the ordinary auto-approved path they submit authorization and execution intent once, then receive a terminal result with bounded artifacts and timing or a durable handle for continuation.",
    responsibilities: [
      "Consumes registration guidance instead of guessing protocol fields from prose.",
      "Uses the unified MCP registry for harness spend calls; direct local-dogfood and diagnostic CLI flows may still use the repository skill's separate explicit-key registry.",
      "Calls hubu_submit_governed_execution with structured authorization and execution intent; optional business task correlation remains trusted client metadata rather than a protected model argument.",
      "Uses the returned public operation handle for correlation; exact harness-call redelivery recovers the same normalized operation, while a distinct call ID always allocates a different operation.",
      ...
      "On approval_required, receives an immediate response with no provider work and uses an existing or external protected human path outside this composite; only exact redelivery of the same composite call resumes after resolution.",
