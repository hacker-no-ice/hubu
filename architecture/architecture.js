...
const components = {
  ...
  budget: {
    ...
    kind: "Component",
    copy:
      "Agent budgets are stable logical allocations whose hard limit lives in an immutable, auditable current version; user spending targets are separate advisory records. Spend reserves exactly one version-attributed hold against the logical balance, converts final exact vendor cost to cents with checked ceiling, and keeps expired or over-limit uncertainty frozen until a human reconciles vendor billing.",
    responsibilities: [
      ...
      "Appends total-limit changes as one immutable direct successor under BEGIN IMMEDIATE, checks the requested edge for exact replay before stale-head rejection, and compare-and-sets the current pointer with the logical balance and status cache in the same transaction.",
      ...
      "Revokes active budgets and preserves the existing replacement contract by creating a separate forward-looking logical budget at revision 1.",
    ...
    nodes: [
      ...
      { id: "reserve", label: "Reserve hold", sub: "logical + version ids", x: 420, y: 248, w: 210, h: 92, tone: "core" },
  ...
  gongbu: {
    ...
    responsibilities: [
      ...
      "Rejects unselectable target tuples, incomplete FLUX pricing profiles, unmatched image-size selectors, and arbitrary or conflicting FLUX dimensions before Hubu resolution, persistence, provider-attempt creation, or provider network activity.",
      ...
      "Persists an immutable execution before scheduling, creates a provider attempt only after preflight and Hubu claim but before provider work, then stores exact integer provider amount, decimal scale, currency, and receipt evidence before finalization and recovery through Gongbu-owned Temporal workflow state; durable normalized and pricing evidence preserve the exact FLUX preset and transmitted dimensions across catalog rotation and restart.",
    ...
    nodes: [
      ...
      { id: "gongbuApi", label: "Execution API", sub: "neutral; freeze target + price + input", x: 340, y: 112, w: 210, h: 92, tone: "executor", path: "crates/gongbu-api/src/http/mod.rs" },
      { id: "workflow", label: "Durable workflow", sub: "preflight → claim → execute → settle", x: 674, y: 112, w: 230, h: 92, tone: "executor", path: "crates/gongbu-api/src/workflow.rs" },
      { id: "executionDb", label: "Gongbu SQLite", sub: "exact receipts + frozen replay", x: 340, y: 352, w: 210, h: 96, tone: "data", path: "crates/gongbu-api/src/execution/mod.rs" },
      ...
      { id: "provider", label: "Provider adapter", sub: "frozen request fields only", x: 674, y: 474, w: 230, h: 92, tone: "executor", path: "crates/gongbu-api/src/provider/mod.rs" },
    ...
    edges: [
      ...
      ["gongbuApi", "hubu", "after freeze: attribution", { fromSide: "right", toSide: "top", waypoints: [{ x: 580, y: 158 }, { x: 580, y: 450 }, { x: 1109, y: 450 }], labelSegment: 2, labelDy: -12 }],
      ["gongbuApi", "executionDb", "frozen replay / persist"],
      ["gongbuApi", "workflow", "schedule frozen request"],
      ["workflow", "temporal", "durable state"],
      ["workflow", "executionDb", "exact cost + frozen snapshot", { labelDx: -36, labelDy: 26 }],
      ["workflow", "hubu", "finalize / ceil-settle / reconcile", { fromSide: "right", toSide: "left", waypoints: [{ x: 936, y: 158 }, { x: 936, y: 606 }], labelSegment: 1, labelDx: 120 }],
      ["workflow", "provider", "execute claimed frozen request once", { fromSide: "left", toSide: "left", waypoints: [{ x: 630, y: 158 }, { x: 630, y: 520 }], labelSegment: 1, labelDx: -80 }],
      ["credentials", "provider", "resolve secret", { labelDx: -175 }],
      ["provider", "vendor", "exact provider fields", { fromSide: "right", toSide: "left", waypoints: [{ x: 962, y: 520 }, { x: 962, y: 182 }], labelSegment: 1 }],
  ...
  mcp: {
    ...
    viewBox: "0 0 1280 760",
    copy:
      "The agent harness launches one default stdio server. The router offers bounded governed submission plus public-handle resume: primitive resume is Hubu-only, governed resume can continue stored Gongbu intent, and completed operations replay from the local registry. Governance, provider execution, backend storage, credentials, artifacts, and failures remain with their owners.",
    responsibilities: [
      ...
      "Publishes the four accepted gongbu_* execution and artifact primitives, local hubu_operation_status, and router-owned hubu_submit_governed_execution and hubu_resume_operation with stable schemas, private continuation binding, public-handle correlation, and recursive redaction.",
      ...
      "Uses only fixed Hubu routes and the Hubu credential, sends the separate approval capability only on protected approval resolution, and sends the separate reconciliation capability only on the two reconciliation mutations.",
      ...
      "Persists one stable local installation identity and atomically allocates an independent private backend operation key and immutable public operation_handle alongside a canonical tool-and-argument request hash and bounded immutable request intent; exact redelivery and handle resume recover while identity collisions fail before backend access.",
    ...
    nodes: [
      ...
      { id: "tools", label: "Static router", sub: "38 tools; submit + resume", x: 330, y: 92, w: 200, h: 96, tone: "surface", path: "crates/hubu-unified-mcp/src/lib.rs" },
      ...
      { id: "operationRegistry", label: "Operation registry", sub: "approval + intent + replay", x: 650, y: 334, w: 200, h: 96, tone: "data", path: "crates/hubu-unified-mcp/src/operation_registry.rs" },
    ...
    edges: [
      ...
      ["tools", "hubuClient", "review + resolve"],
