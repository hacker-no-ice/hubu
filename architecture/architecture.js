...
const components = {
  ...
  mcp: {
    ...
    viewBox: "0 0 1280 760",
    copy:
      "The agent harness launches one default stdio server. That router probes and calls two isolated HTTP backends through separate clients and credentials; it owns discovery, routing, and local harness-operation identity state, not governance, provider execution, backend storage, or artifacts.",
    responsibilities: [
      ...
      "Keeps operation_key and task_id out of model-visible spend schemas, normalizes bounded Codex, Claude Code, or controlled Hubu metadata, and injects resolved identities into the HTTP request.",
      "Persists one stable local installation identity and atomically allocates private namespaced operation keys from canonical tool-and-argument hashes; exact redelivery recovers while identity collisions fail before backend access.",
    ...
    nodes: [
      ...
      { id: "operationRegistry", label: "Operation registry", sub: "local identity + SQLite", x: 650, y: 334, w: 200, h: 96, tone: "data", path: "crates/hubu-unified-mcp/src/operation_registry.rs" },
    ...
    edges: [
      ...
      ["tools", "operationRegistry", "spend identity"],
      ["operationRegistry", "hubuClient", "private key"],
  ...
  agent: {
    ...
    responsibilities: [
      ...
      "Reuses one unified-MCP-owned operation key throughout an exact harness-call redelivery and allocates a different key for a distinct call ID.",
    ...
    nodes: [
      ...
      { id: "operation", label: "Operation registry", sub: "normalized call + stable key", x: 90, y: 336, w: 220, h: 92, tone: "data", path: "crates/hubu-unified-mcp/src/operation_registry.rs" },
