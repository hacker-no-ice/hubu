...
const components = {
  ...
  mcp: {
    ...
    responsibilities: [
      ...
      "Persists one stable local installation identity and atomically allocates a private backend operation key plus an independent immutable public operation_handle from canonical tool-and-argument hashes; exact redelivery recovers while identity collisions fail before backend access.",
      "Marks dispatch before Hubu mutation, stores the sanitized replay payload separately from decision and continuation columns, recursively removes operation_key from every agent-facing response location, and erases terminal private keys.",
