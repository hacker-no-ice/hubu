...
const components = {
  top: {
    ...
    nodes: [
      ...
      { id: "app", label: "App services", sub: "spend orchestration", x: 620, y: 360, w: 190, h: 98, tone: "core", path: "crates/hubu-core/src/app/spend_approval.rs" },
  ...
  api: {
    ...
    nodes: [
      ...
      { id: "governance", label: "Governance DB", sub: "policy/budget/claims", x: 804, y: 180, w: 196, h: 84, tone: "data" },
  ...
  app: {
    ...
    kind: "Component",
    copy:
      "The core app layer coordinates managers and repositories for use cases that need more than one domain object. Spend approval now runs here so the authorize/reserve/payment/settle path can be tested directly without exercising HTTP routes.",
    responsibilities: [
      ...
      "Returns domain-shaped approval, rejection, and settlement results while the API owns public IDs and JSON response shape.",
    ],
    links: [sharedLinks.appSpend, sharedLinks.spend, sharedLinks.budget, sharedLinks.persistence, sharedLinks.payment, sharedLinks.paymentAttempt],
    nodes: [
      { id: "input", label: "Use-case input", sub: "internal IDs + policy", x: 78, y: 112, w: 230, h: 92, tone: "core" },
      { id: "policy", label: "Policy evaluate", sub: "SpendManager", x: 430, y: 92, w: 224, h: 92, tone: "core", path: "crates/hubu-core/src/spend/manager.rs" },
      { id: "reserve", label: "Reserve budget", sub: "one agent hold", x: 430, y: 282, w: 224, h: 92, tone: "core", path: "crates/hubu-core/src/budget/manager.rs" },
      { id: "persist", label: "Persist governance", sub: "decision/token/hold", x: 798, y: 188, w: 238, h: 98, tone: "data", path: "crates/hubu-core/src/persistence.rs" },
      { id: "payment", label: "Payment submit", sub: "wallet boundary", x: 430, y: 482, w: 224, h: 92, tone: "wallet", path: "crates/hubu-wallet/src/payment.rs" },
      { id: "settle", label: "Settle/release", sub: "agent hold", x: 798, y: 444, w: 238, h: 98, tone: "core", path: "crates/hubu-core/src/app/spend_approval.rs" },
    ],
    edges: [
      ["input", "policy", "authorize"],
      ["policy", "reserve", "allow"],
      ["reserve", "persist", "save"],
      ["persist", "payment", "token"],
      ["payment", "settle", "status"],
      ["settle", "persist", "update"],
    ],
  },
  registration: {
    title: "Registration",
  ...
  budget: {
    ...
    links: [sharedLinks.budget, sharedLinks.budgetModel, sharedLinks.spendingTarget, sharedLinks.appSpend, sharedLinks.spendExecutor, sharedLinks.persistence, ["Budget DTOs", "crates/hubu-core/src/budget/dto.rs"]],
    nodes: [
  ...
  gongbu: {
    ...
    copy:
      "Gongbu is outside Hubu. Agents ask it to perform work, Gongbu exclusively claims the scoped Hubu authorization, calls external vendors, and finalizes the claim through settle or release.",
    responsibilities: [
    ...
    nodes: [
      ...
      { id: "hubu", label: "Hubu", sub: "claim + finalize", x: 446, y: 462, w: 230, h: 92, tone: "core" },
    ],
    edges: [
  ...
  mcp: {
    ...
    responsibilities: [
      ...
      "Hides operation_key from spend tool arguments and derives it from trusted per-operation MCP metadata plus the configured platform namespace.",
      "Fails closed when operation metadata is absent, malformed, or overridden by model-controlled arguments.",
    ...
    nodes: [
      { id: "agent", label: "Agent client", sub: "arguments + trusted meta", x: 50, y: 134, w: 220, h: 92, tone: "agent" },
      { id: "tools", label: "Tool catalog", sub: "key hidden from model", x: 340, y: 134, w: 230, h: 92, tone: "core" },
      { id: "operation", label: "Operation adapter", sub: "namespace + metadata id", x: 710, y: 134, w: 240, h: 92, tone: "core" },
      { id: "approval", label: "Approval gate", sub: "trusted env flag", x: 340, y: 356, w: 230, h: 92, tone: "human" },
      { id: "api", label: "HTTP forwarder", sub: "bearer + Hubu", x: 710, y: 356, w: 240, h: 92, tone: "core" },
    ],
    edges: [
      ["agent", "tools", "list/call"],
      ["tools", "approval", "protected"],
      ["approval", "api", "allowed"],
      ["tools", "operation", "spend"],
      ["operation", "api", "inject key"],
      ["tools", "api", "read"],
    ],
  },
  agent: {
    ...
    responsibilities: [
      ...
      "Relies on the client platform to attach one stable logical-operation id outside model-controlled tool arguments.",
