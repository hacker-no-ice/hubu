...
const components = {
  top: {
    ...
    responsibilities: [
      ...
      "Agents discover Hubu through configured MCP tools; trusted client invocation metadata is resolved through the adapter's durable operation-key registry before spend reaches the API, while humans use the CLI for setup and administration.",
    ...
    nodes: [
      ...
      { id: "mcp", label: "MCP adapter", sub: "trusted key injection", x: 334, y: 330, w: 184, h: 88, tone: "surface" },
  ...
  mcp: {
    ...
    kind: "Interface",
    copy:
      "The MCP stdio adapter exposes Hubu as agent tools that Codex and other MCP clients can discover after configuration. For spend calls it resolves trusted client invocation metadata to a durable opaque operation key outside model control. Read-only calls are safe to inspect; protected setup tools require trusted client approval.",
    responsibilities: [
      ...
      "Keeps operation_key out of spend schemas, rejects spoofed model arguments, and requires trusted platform, installation, and logical invocation metadata from the MCP client.",
      "Persists invocation-to-key mappings in its own SQLite registry so retries and process recovery reuse one key while separate provider candidates remain distinct.",
      "Loads the local Hubu token, injects the resolved key into spend requests, forwards calls to the HTTP API, and marks needs_approval responses for the agent client.",
    ],
    links: [sharedLinks.mcp, ["MCP transport doc", "docs/mcp-transport.md"], sharedLinks.api],
    nodes: [
      { id: "agent", label: "Agent client", sub: "args + trusted metadata", x: 60, y: 116, w: 238, h: 92, tone: "agent" },
      { id: "tools", label: "Tool catalog", sub: "no model operation key", x: 416, y: 96, w: 244, h: 92, tone: "core" },
      { id: "registry", label: "Invocation registry", sub: "durable opaque keys", x: 416, y: 256, w: 244, h: 92, tone: "data", path: "crates/hubu-mcp/src/operation_keys.rs" },
      { id: "approval", label: "Approval gate", sub: "trusted env flag", x: 416, y: 428, w: 244, h: 92, tone: "human" },
      { id: "api", label: "HTTP forwarder", sub: "key + bearer", x: 802, y: 256, w: 220, h: 92, tone: "core" },
    ],
    edges: [
      ["agent", "tools", "args + metadata"],
      ["tools", "registry", "resolve/allocate"],
      ["registry", "api", "inject key"],
  ...
  agent: {
    ...
    responsibilities: [
      ...
      "Supplies spend intent while the trusted platform adapter owns and injects the stable operation key outside model-visible arguments.",
