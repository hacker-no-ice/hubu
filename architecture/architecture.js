const sharedLinks = {
  ...
  ledger: ["Wallet ledger", "crates/hubu-wallet/src/ledger.rs"],
  providerAccounting: ["Provider accounting", "crates/hubu-core/src/persistence/accounting.rs"],
...
const components = {
  ...
  api: {
    ...
    responsibilities: [
      ...
      "Uses SQLite as the finalization authority so exact receipt, balanced non-cash provider posting, conservative budget charge, overrun, claim, token, hold, and balance commit atomically, settle serializes against release, and identical executor or human reconciliation retries return stored state.",
  ...
  budget: {
    ...
    responsibilities: [
      ...
      "Enforces unique agent-scoped operation ownership and finalizes exact receipt, balanced provider journal pair, conservative cent charge, claim, token, hold, and budget balance in one immediate SQLite transaction while leaving expired or executor-overrun claims frozen for reconciliation.",
  ...
  ledger: {
    ...
    kind: "Component",
    copy:
      "Hubu owns wallet cash accounting and a separate non-cash provider subledger. Provider expense and externally billed clearing post atomically with receipt and budget settlement; neither requires Gongbu.",
    responsibilities: [
      ...
      "Prevents updates and deletes for wallet entries and provider journal records. Each provider record exposes one balanced pair of exact scale-18 lines.",
      "Preserves provider cost separately from budget rounding; corrections append linked records through BudgetManager and publish committed budget state. Legacy backfill preserves evidence and marks missing coverage without consuming budget again.",
    ...
    links: [sharedLinks.ledger, sharedLinks.providerAccounting, sharedLinks.payment, ["Provider accounting", "docs/provider-accounting.md"], ["Wallet persistence", "crates/hubu-wallet/src/persistence.rs"]],
    nodes: [
      { id: "providerJournal", label: "Provider journal", sub: "expense + external clearing", x: 70, y: 580, w: 260, h: 92, tone: "data", path: "crates/hubu-core/src/persistence/accounting.rs" },
      ...
      { id: "correction", label: "Corrections", sub: "append + reconcile", x: 790, y: 580, w: 250, h: 92, tone: "core", path: "crates/hubu-core/src/budget/manager.rs" },
