...
const components = {
  top: {
    ...
    copy:
      "Agents use one default MCP surface. One installation-scoped Gongbu caller reaches executions for multiple owner agents, while Hubu authorization supplies per-execution attribution. The independently operated backends retain separate credentials, storage, provider work, artifacts, and failure domains.",
    responsibilities: [
  ...
  mcp: {
    ...
    viewBox: "0 0 1280 760",
    copy:
      "The agent harness launches one default stdio server. That router probes and calls two isolated HTTP backends through separate clients and credentials; it owns discovery, routing, and the durable normalized-operation lifecycle, not governance, provider execution, backend storage, or artifacts.",
    responsibilities: [
      ...
      "Publishes and routes the four accepted gongbu_* execution and artifact tools plus local hubu_operation_status with stable schema, private continuation binding, public-handle correlation, and recursive redaction.",
      ...
      "Rejects unknown and out-of-map tool calls before domain network access, never falls back across backends, never retries provider mutations, and sanitizes backend outages.",
      ...
      "Binds each allowed auth_token_id to exactly one canonical Gongbu create intent before backend access, temporarily persists the validated request for restart-safe replay, deletes it after execution identity is recovered, persists Gongbu execution identity plus lifecycle state, and fails closed on changed intent, spoofed protected controls, or mismatched returned identity.",
      ...
      "Treats the registry as an independent billable-operation capability: missing or broken state hides and rejects new Hubu spend calls without stopping the router or affecting Hubu reads, Gongbu reads, status, or artifact access.",
    ...
    nodes: [
      ...
      { id: "tools", label: "Static router", sub: "28 Hubu + 4 Gongbu + 2 local", x: 330, y: 92, w: 200, h: 96, tone: "surface", path: "crates/hubu-unified-mcp/src/lib.rs" },
    ...
    edges: [
      ["agent", "tools", "stdio + public handle", { labelDy: -54 }],
  ...
  agent: {
    ...
    kind: "Flow",
    copy:
      "Agents never hold private backend operation keys. They register as distinct accounts, operate under the current user's policy and their own hard budget, and submit spend intent for Hubu to authorize. User spending targets remain advisory to humans.",
    responsibilities: [
    ...
    nodes: [
      ...
      { id: "operation", label: "Operation registry", sub: "normalized call + public handle", x: 90, y: 336, w: 220, h: 92, tone: "data", path: "crates/hubu-unified-mcp/src/operation_registry.rs" },
    ...
    edges: [
      ...
      ["operation", "spend", "private backend identity"],
